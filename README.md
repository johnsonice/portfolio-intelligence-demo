...
The product direction developed on `explore/priority-led-home` has been merged into [`main`](https://github.com/johnsonice/portfolio-intelligence-demo/tree/main). Main retains its original Sites project configuration; the exploration branch retains its separate deployment. The published versions below identify the source each currently serves.

## Published demos

| Version | Live website | Published source |
| --- | --- | --- |
...
| Original demo | [Open the original demo](https://portfolio-intelligence-demo.johnsonice.chatgpt.site/) | [`ce07a92`](https://github.com/johnsonice/portfolio-intelligence-demo/tree/ce07a9226e918158bb69b06828033f3804c9190d) |
...
- **Home:** two compact sections, **To review** and **Suggested tasks**. Existing findings and completed analyses use **Review**; unexecuted recommendations use **Run**, with **Adjust** available before execution. The original company finding and later results from the same investigation share one card. Portfolio-wide findings retain their separate report links and are shown for All companies. Both sections follow the company selection; completed suggestions move to To review. Collapsed previews keep the first view concise; expanding reveals a small chart or the proposed plan. Running, paused and data-waiting tasks appear first in the existing **Continue your work** area, with their status and next step. Opening one resumes its existing session without automatically restarting execution. There is no separate review-queue banner. Earlier reports, conversations and saved outputs remain available in Cases and Workspace.
